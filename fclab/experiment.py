...
from .fclab import FCLab, FCLabModel
from .historian import Historian, Plotter
from .labtime import labtime, clock


class Experiment:
    """Utility class for running experiments on the FCLab

    The class is designed to be used as a context. On initialisation
    it will automatically connect to the FCLab, create a Historian and Plotter
    ...
    FCLab version ...
    Simulated FCLab
    FCLab Model disconnected successfully.
    ...
    def __init__(self, connected=True, plot=True,
        ...
        """Parameters:
        connected: If True, connect to a physical FCLab, if False, connecte to
                   FCLabModel
        plot: Use a Plotter
        ...
        if (speedup != 1 or not synced) and connected:
            raise ValueError('The real FCLab can only run real time.')
    ...
    def __enter__(self):
        if self.connected:
            ...
            self.lab = FCLabModel(synced=self.synced)
...
def runexperiment(function, *args, **kwargs):
    ...
    The function will be passed the time and a FCLab instance at every tick of
    the clock.
