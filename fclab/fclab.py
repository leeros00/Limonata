...
@author: ltlee
"""
# TO DO: Anywhere there is a pumpQ and controlValveQ, replace them with pumpQ, valveQ, agitatorQ
...
sep = ' '   # command/value separator in FCLab firmware

arduinos = [('USB VID:PID=16D0:0613', 'Arduino Uno'),
			('USB VID:PID=2341:0043', 'Arduino Uno Celeste'),
...
#_sketchurl = 'https://github.com/jckantor/FCLab-sketch'
_connected = False
...
def command(name, argument, lower=0, upper=100):
    """Construct command to FCLab-sketch."""
...
class FCLab(object):
    def __init__(self, port='', debug=False):
        global _connected
        self.debug = debug
        print("FCLab version", __version__)
        ...
        try:
            ...
            try:
                ...
                print('This may be due to an old FCLab firmware.')
                print('New Arduino FCLab firmware available at:')
                # # print(_sketchurl)
            except:
    ...
    def connect(self, baud):
        ...
        time.sleep(2)
        self.pumpQ(0)  # fails if not connected
        self.baud = baud

    def close(self):
        """Shut down FCLab device and close serial connection."""
        global _connected
        ...
        self.sp.close()
        _connected = False
        print('FCLab disconnected successfully.')
        return

    def send(self, msg):
        """Send a string message to the FCLab firmware."""
        self.sp.write((msg + '\r\n').encode())
        if self.debug:
            print('Sent: "' + msg + '"')
        self.sp.flush()
    ...
    def receive(self):
        """Return a string message received from the FCLab firmware."""
    ...
    def LED(self, val=100):
        """Flash FCLab LED at a specified brightness for 10 seconds."""
        return self.send_and_receive(command('LED', val), float)

    @property
    def T1(self):
        """Return a float denoting FCLab temperature T1 in degrees C."""
        return self.send_and_receive('T1', float)

    @property
    def T2(self):
        """Return a float denoting FCLab temperature T2 in degrees C."""
    ...
    def pumpQ(self, val=None):
        """Get or set FCLab heater power pumpQ
    ...
    def controlValveQ(self, val=None):
        """Get or set FCLab heater power controlValveQ
        ...
        if val is None:
            ...
            msg = 'controlValveQ' + sep + str(clip(val))
        return self.send_and_receive(msg, float)
    ...
    def scan(self):
        ...
        pumpQ = self.pumpQ()  # float(self.receive())
        controlValveQ = self.controlValveQ()  # float(self.receive())
        return T1, T2, pumpQ, controlValveQ

    # Define properties for pumpQ and controlValveQ
    U1 = property(fget=pumpQ, fset=pumpQ, doc="Heater 1 value")
    U2 = property(fget=controlValveQ, fset=controlValveQ, doc="Heater 2 value")


class FCLabModel(object):
    def __init__(self, port='', debug=False, synced=True):
        self.debug = debug
        self.synced = synced
        print("FCLab version", __version__)
        labtime.start()
        print('Simulated FCLab')
        ...
        self._controlValveQ = 100                  # initial heater 2
    ...
    def close(self):
        """Simulate shutting down FCLab device."""
        self.pumpQ(0)
        self.controlValveQ(0)
        print('FCLab Model disconnected successfully.')
        return

    def LED(self, val=100):
        """Simulate flashing FCLab LED
    ...
    def pumpQ(self, val=None):
        """Get or set FCLabModel heater power pumpQ
    ...
    def controlValveQ(self, val=None):
        """Get or set FCLabModel heater power controlValveQ
        ...
        self.update()
        if val is not None:
            self._controlValveQ = clip(val)
        return self._controlValveQ
    ...
    def update(self, t=None):
        ...
        while teuler < self.tnow:
            ...
            dH1 = self._P1 * self._pumpQ / 5720 + DeltaTaH1 / 20 - DeltaT12 / 100
            dH2 = self._P2 * self._controlValveQ / 5720 + DeltaTaH2 / 20 + DeltaT12 / 100
...
def diagnose(port=''):
    ...
    heading('Testing FCLab object in debug mode')

    with FCLab(port=port, debug=True) as lab:
        print('Reading Flow Rate')
        print(lab.valveQ)

    heading('Testing FCLab functions')

    with FCLab(port=port) as lab:
        ...
        if pumpQ != 0.5:
            print("Your FCLab firmware version ({}) doesn't support"
